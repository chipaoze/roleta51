...
function renderCommunityFeed(wall) {
  ...
  const signature = JSON.stringify([posts, emojis, avatars, appState?.me?.avatarDataUrl || null]);
  ...
  container.innerHTML = posts.map((post) => {
    ...
    <header><div class="feed-author">${avatar}<div><strong>${anonymous ? '🕵️ Anônimo' : escapeHtml(authorName)}</strong><time>${escapeHtml(formatDate(post.createdAt))}</time></div></div>
