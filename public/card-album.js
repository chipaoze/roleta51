...
function renderCardTrades(trading){
  ...
  const albumCards=(appState.cardAlbum?.collections || []).flatMap(collection=>collection.cards),mineCounts=Object.fromEntries(albumCards.map(card=>[card.id,Number(card.count||0)])),catalog=new Map(albumCards.map(card=>[card.id,card]));
