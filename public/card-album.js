...
function renderCardTrades(trading){
  ...
  const mineCounts=trading.partners?.find(person=>person.id===appState.me.id)?.ownedCardCounts || {},catalog=new Map((appState.cardAlbum?.collections || []).flatMap(collection=>collection.cards.map(card=>[card.id,card])));
