...
function renderDirectTradeChoices(){
  ...
  const valueForCollection=(card,count)=>{const info=collectionInfo.get(card.id)||{},collection=info.collection||{},remaining=Math.max(0,5-(Number(collection.collected||0)+(count?0:1))),rare=info.rarity==='rare';if(!count)return (rare?'★ Rara · ':'✦ Nova · ')+(remaining?'faltam '+remaining+' para a insígnia':'completa esta coleção');if(count===1)return (rare?'★ Rara · ':'')+'você já tem 1 · extra pode ajudar na próxima insígnia';return (rare?'★ Rara · ':'')+'você já tem '+count+' · pouco impacto agora';};
  ...
  const useful=catalog.filter(card=>Number(theirs[card.id]||0)>=2&&mineCounts[card.id]===0).map(card=>({...card,detail:'ele tem '+theirs[card.id]+' cópias'}));
