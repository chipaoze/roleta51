...
function renderCardTrades(trading){
  ...
  const visualChoices=(items,name,emptyText,selectedId='')=>{const chosen=items.some(card=>card.id===selectedId)?selectedId:items[0]?.id,selected=items.find(card=>card.id===chosen);return items.length?'<div class="card-choice-combo"><span class="card-choice-combo-icon" aria-hidden="true">'+escapeHtml(selected?.icon || '🎴')+'</span><label><small>Escolha a carta repetida</small><select name="'+name+'">'+items.map(card=>'<option value="'+escapeHtml(card.id)+'" data-icon="'+escapeHtml(card.icon)+'"'+(card.id===chosen?' selected':'')+'>'+escapeHtml(card.icon+' '+card.name)+' · '+Number(card.freeCopies||0)+' cópia'+(Number(card.freeCopies||0)===1?' livre':'s livres')+'</option>').join('')+'</select></label><b>'+Number(selected?.count||0)+' cópias no álbum</b></div>':'<p class="card-choice-empty">'+emptyText+'</p>';};
  ...
  document.querySelector('#cardTradeList').innerHTML=(trading.trades || []).map(t=>{const sent=t.incoming?{name:t.wantedName,icon:t.wantedIcon,count:t.viewerWantedCount}:{name:t.offeredName,icon:t.offeredIcon,count:t.viewerOfferedCount};const received=t.incoming?{name:t.offeredName,icon:t.offeredIcon,count:t.viewerOfferedCount}:{name:t.wantedName,icon:t.wantedIcon,count:t.viewerWantedCount};const receiveNote=received.count?'Você já tem '+received.count+' cópia'+(Number(received.count)>1?'s':''):'Nova para sua coleção';const sendNote=sent.count>1?'Sua carta repetida · '+sent.count+' cópias':'Sua cópia reservada';return '<article class="card-trade-status-card"><header><strong>'+escapeHtml(t.partnerName)+'</strong><small>'+escapeHtml(labels[t.status] || t.status)+'</small></header><div class="card-trade-status-deal"><section><b>VOCÊ ENVIA</b><span>'+escapeHtml(sent.icon)+'</span><p><strong>'+escapeHtml(sent.name)+'</strong><small>'+escapeHtml(sendNote)+'</small></p></section><i aria-hidden="true">⇄</i><section class="received '+(received.count?'already-owned':'')+'"><b>VOCÊ RECEBE</b><span>'+escapeHtml(received.icon)+'</span><p><strong>'+escapeHtml(received.name)+'</strong><small>'+escapeHtml(receiveNote)+'</small></p></section></div>'+(t.status==='pending'?'<footer>'+(t.incoming?'<button data-card-trade="accept" data-id="'+escapeHtml(t.id)+'">Aceitar</button><button data-card-trade="reject" data-id="'+escapeHtml(t.id)+'">Recusar</button>':'<button data-card-trade="cancel" data-id="'+escapeHtml(t.id)+'">Cancelar</button>')+'</footer>':'')+'</article>';}).join('') || '<p>Nenhuma proposta. Encontre cartas repetidas para começar a trocar.</p>';
  const marketForm=document.querySelector('#cardMarketForm'),mine=trading.partners?.find(p=>p.id===appState.me.id)?.cards || [];
  const availableToPublish=mine.filter(card=>card.available!==false);
  ...
  document.querySelector('#cardTradeMarket').innerHTML=(trading.market || []).map(post=>{
    const eligibleMine=mine.filter(card=>card.available!==false);
...
function renderDirectTradeChoices(){
  ...
  const tradeSelector=(items,role,selectedId,emptyText)=>{const available=(items || []).filter(card=>card.available!==false),chosen=available.find(card=>card.id===selectedId)||available[0];if(!chosen)return '<p class="card-choice-empty">'+emptyText+'</p>';const receiving=role==='want',already=Number(chosen.viewerCount||0)>0,detail=receiving?(already?'Você já possui '+chosen.viewerCount+' cópia'+(Number(chosen.viewerCount)>1?'s':'')+' desta carta.':'Carta nova para sua coleção.'):(chosen.count+' cópias no álbum · '+chosen.freeCopies+' livre'+(Number(chosen.freeCopies)===1?'':'s')+' para negociar.');return '<section class="direct-trade-selector '+role+'"><label><small>'+ (receiving?'VOCÊ RECEBE':'VOCÊ ENVIA') +'</small><select data-direct-trade-select="'+role+'">'+available.map(card=>'<option value="'+escapeHtml(card.id)+'"'+(card.id===chosen.id?' selected':'')+'>'+escapeHtml(card.icon+' '+card.name)+' · '+(receiving?(Number(card.viewerCount||0)?'você já tem':'nova'):(card.freeCopies+' livre'+(Number(card.freeCopies)===1?'':'s')))+'</option>').join('')+'</select></label><article class="direct-trade-focus '+(already?'already-owned':'')+'"><span>'+escapeHtml(chosen.icon)+'</span><p><strong>'+escapeHtml(chosen.name)+'</strong><small>'+escapeHtml(detail)+'</small></p></article></section>';};
  ...
  const cardsIn=(items,empty)=>items.length?items.map(card=>'<span title="'+escapeHtml(card.collectionName)+'"><b>'+escapeHtml(card.icon)+'</b>'+escapeHtml(card.name)+'<small>'+escapeHtml(card.detail)+'</small></span>').join(''):'<em>'+empty+'</em>';
  ...
  const comparison='<section class="card-trade-comparison" aria-label="Comparação de coleções"><header><div><small>COMPARADOR DE COLEÇÃO</small><strong>Você e '+escapeHtml(partner.name)+'</strong></div><span>'+Object.keys(theirs).length+' cartas dele</span></header><div><article class="need"><b>✦ Posso pedir</b>'+cardsIn(useful,'Nenhuma repetida nova disponível.')+'</article><article class="owned"><b>✓ Já tenho</b>'+cardsIn(already,'Nenhuma carta em comum ainda.')+'</article><article class="shared"><b>↔ Repetidas em comum</b>'+cardsIn(shared,'Sem repetidas iguais.')+'</article></div></section>';
  host.innerHTML='<section class="direct-trade-deal" aria-label="Resumo da proposta com '+escapeHtml(partner.name)+'"><header><small>PROPOSTA PARA '+escapeHtml(partner.name).toUpperCase()+'</small><strong>Escolha as duas cartas da troca</strong></header><div>'+tradeSelector(mine?.cards,'offer',form.elements.offeredId.value,'Você ainda não tem cartas repetidas livres.')+'<b class="direct-trade-arrow" aria-hidden="true">⇄</b>'+tradeSelector(partner.cards,'want',form.elements.wantedId.value,escapeHtml(partner.name)+' não tem cartas repetidas disponíveis agora.')+'</div></section><details class="direct-trade-insights"><summary>Ver comparador completo de coleção</summary>'+comparison+'</details>';
}
document.querySelector('#cardTradeForm').addEventListener('change',e=>{const form=e.currentTarget;if(e.target.name==='partnerId'){updateCardTradeWanted();renderDirectTradeChoices();}if(e.target.dataset.directTradeSelect==='offer'){form.elements.offeredId.value=e.target.value;renderDirectTradeChoices();}if(e.target.dataset.directTradeSelect==='want'){form.elements.wantedId.value=e.target.value;renderDirectTradeChoices();}});
...
function refreshCardChoiceCombo(select){const combo=select.closest('.card-choice-combo'),option=select.selectedOptions?.[0];if(!combo||!option)return;const icon=combo.querySelector('.card-choice-combo-icon');if(icon)icon.textContent=option.dataset.icon||'🎴';const count=combo.querySelector('b');if(count){const text=option.textContent.match(/·\s*(\d+) cópia/);if(text)count.textContent=text[1]+' cópias no álbum';}}
