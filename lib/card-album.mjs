...
export function albumFor(db,userId,dayKey='') {
  ...
  const craftedOrder=Object.entries(saved.crafted || {}).sort((a,b)=>String(a[1]).localeCompare(String(b[1]))).map(([id])=>id);
  const medalFor=(collectionId)=>{const order=craftedOrder.indexOf(collectionId)+1;if(!order)return null;return order===1?{tier:'bronze',label:'Bronze',icon:'🥉',order}:order===2?{tier:'silver',label:'Prata',icon:'🥈',order}:order===3?{tier:'gold',label:'Ouro',icon:'🥇',order}:{tier:'diamond',label:'Diamante',icon:'💎',order};};
  return {distributionEnabled:true,equipped:saved.equipped || null,dropsToday:saved.daily?.dayKey===dayKey?saved.daily.drops:0,attemptsToday:saved.daily?.dayKey===dayKey?Object.keys(saved.daily.attempts || {}).length:0,recentDrops:saved.drops || [],trading:cardTradingFor(db,userId),collections:CARD_COLLECTIONS.map(c=>{
    ...
    return {...c,cards,craftedAt,medal:medalFor(c.id),collected:cards.filter(c=>c.count>0).length,canCraft:!craftedAt && cards.every(c=>c.count>0)};
  })};
}
...
function reservedCardCopies(db,userId,cardId,ignored={}) {
  ...
  const direct=(db.economy.cardTrades || []).filter(trade=>trade.status==='pending'&&Date.parse(trade.expiresAt)>Date.now()&&trade.fromId===userId&&trade.offeredId===cardId&&trade.id!==ignored.tradeId).length;
...
export function updateCardTrade(db,userId,body,idFactory) {
  ...
  if(body.action==='accept'){
    ...
    if(!hasFreeDuplicate(db,trade.fromId,trade.offeredId,{tradeId:trade.id}) || !hasFreeDuplicate(db,trade.toId,trade.wantedId))fail('Uma carta ficou reservada ou deixou de ser repetida. Recuse a proposta e crie outra.');
