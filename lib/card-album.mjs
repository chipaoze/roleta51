...
export function cardTradingFor(db,userId) {
  ...
  const cardsFor=ownerId=>Object.entries(db.economy.cardAlbums?.[ownerId]?.cards || {}).filter(([id,n])=>catalog.has(id)&&count(n)>=2).map(([id,n])=>{
    const copies=count(n),reservedCopies=reservedCardCopies(db,ownerId,id),freeCopies=Math.max(0,copies-1-reservedCopies);
    return {id,count:copies,reservedCopies,freeCopies,available:freeCopies>0,viewerCount:count(db.economy.cardAlbums?.[userId]?.cards?.[id]),...catalog.get(id)};
  });
  ...
  return {partners:(db.users || []).filter(u=>u.active && u.approved!==false).map(u=>({id:u.id,name:u.displayName,cards:cardsFor(u.id),ownedCardCounts:Object.fromEntries(Object.entries(db.economy.cardAlbums?.[u.id]?.cards || {}).filter(([id,n])=>catalog.has(id)&&count(n)>0).map(([id,n])=>[id,count(n)]))})),market,reservedOfferCardIds,reservedOfferCredits,availableTradeCredits:Math.max(0,wallet-reservedOfferCredits),
    trades:(db.economy.cardTrades || []).filter(t=>t.fromId===userId || t.toId===userId).slice(-20).reverse().map(t=>({...t,incoming:t.toId===userId,partnerName:db.users.find(u=>u.id===(t.fromId===userId?t.toId:t.fromId))?.displayName || 'Conta removida',offeredName:catalog.get(t.offeredId)?.name || t.offeredId,offeredIcon:catalog.get(t.offeredId)?.icon || '🎴',wantedName:catalog.get(t.wantedId)?.name || t.wantedId,wantedIcon:catalog.get(t.wantedId)?.icon || '🎴',viewerOfferedCount:count(db.economy.cardAlbums?.[userId]?.cards?.[t.offeredId]),viewerWantedCount:count(db.economy.cardAlbums?.[userId]?.cards?.[t.wantedId]),viewerHasOfferedCard:count(db.economy.cardAlbums?.[userId]?.cards?.[t.offeredId])>0,viewerHasWantedCard:count(db.economy.cardAlbums?.[userId]?.cards?.[t.wantedId])>0,status:t.status==='pending'&&Date.parse(t.expiresAt)<=Date.now()?'expired':t.status}))};
}
