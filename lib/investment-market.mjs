...
function slotKey(date = new Date()) {
  ...
  // Cinco janelas fixas no fuso de São Paulo (00h, 05h, 10h, 15h e 20h).
  // O sync continua sendo idempotente: somente a primeira chamada da janela
  // persiste uma nova cotação no D1.
  return `${get('year')}-${get('month')}-${get('day')}:s${Math.floor(Number(get('hour')) / 5)}`;
}
...
export function marketForUser(economy, userId) {
  ...
  const assets = MARKET_ASSETS.map((asset) => {
    ...
    return { ...asset, price, previousPrice, change, changePercent: previousPrice ? (change / previousPrice) * 100 : 0, direction: change > 0 ? 'up' : change < 0 ? 'down' : 'flat', quantity: Math.max(0, Number(portfolio[asset.id] || 0)), positionValue: price * Math.max(0, Number(portfolio[asset.id] || 0)) };
  });
