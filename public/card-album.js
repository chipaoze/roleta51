...
function renderDirectTradeChoices(){
  ...
  const comparison='<section class="card-trade-comparison" aria-label="Resumo de troca"><header><div><small>RESUMO DA TROCA</small><strong>O que vale a pena pedir para '+escapeHtml(partner.name)+'</strong></div></header><div class="trade-summary-stats"><span><b>'+useful.length+'</b> cartas novas para você</span><span><b>'+shared.length+'</b> repetidas para os dois</span><span><b>'+Object.keys(theirs).length+'</b> cartas na coleção dele</span></div><article class="trade-opportunities"><header><b>✦ '+escapeHtml(partner.name)+' tem repetidas que você ainda não possui</b><small>Escolha uma delas em “Preciso receber” acima.</small></header><div>'+cardsIn(useful,'Nenhuma oportunidade nova disponível agora.')+'</div></article><details class="trade-comparison-details"><summary>Ver cartas que vocês já têm em comum ('+already.length+')</summary><div>'+cardsIn(already,'Nenhuma carta em comum ainda.')+'</div></details></section>';
